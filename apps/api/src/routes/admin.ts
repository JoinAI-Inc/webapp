...
router.get('/features', async (req: Request, res: Response) => {
    try {
        ...
        const serializedFeatures = JSON.parse(JSON.stringify(features, (key, value) =>
            typeof value === 'bigint' ? value.toString() : value
        ));
...
router.post('/features', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const serializedFeature = JSON.parse(JSON.stringify(feature, (key, value) =>
            typeof value === 'bigint' ? value.toString() : value
        ));
        res.json(serializedFeature);
    } catch (error: any) {
        res.status(400).json({ error: error.message });
    }
});
